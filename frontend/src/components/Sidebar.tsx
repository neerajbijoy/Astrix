...
import { LogOut, Shield, LayoutDashboard, FileSpreadsheet, PlusCircle, History, ClipboardCheck, Sparkles, BookOpen } from 'lucide-react';
import { DemoUser } from '../pages/Auth';
...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, demoUser, onLogout }) => {
  const navItems = [
    { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard },
    { id: 'claims', label: 'Claims', icon: FileSpreadsheet },
    { id: 'new-audit', label: 'New Audit', icon: PlusCircle, highlight: true },
    { id: 'history', label: 'Audit History', icon: History },
    { id: 'cdt-library', label: 'CDT Library', icon: BookOpen },
    { id: 'settings', label: 'Claim Outcomes', icon: ClipboardCheck },
  ];
      ...
      <div className="p-4 border-t border-slate-800 space-y-3">
        ...
        <div className="flex items-center justify-between gap-3 px-2 py-1">
          ...
          <div className="w-8 h-8 rounded-full bg-brand-600 text-white font-bold text-xs flex items-center justify-center font-mono">
            {demoUser.role.slice(0, 2)}
          </div>
          <div className="min-w-0">
            <p className="text-xs font-bold text-white truncate">{demoUser.displayName}</p>
            <p className="text-[10px] text-slate-400 truncate">{demoUser.role}</p>
          </div>
          </div>
          <button type="button" onClick={onLogout} title="Sign out" className="p-2 text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl">
