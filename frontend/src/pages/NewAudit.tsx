...
import { fetchPayers, fetchClaimById, createClaimApi, updateClaimApi, uploadDocumentApi, runAuditApi } from '../services/api';

interface NewAuditProps {
  onAuditComplete: (auditResult: any, claimId: string) => void;
  existingClaimId?: string | null;
  initialCdtCode?: string | null;
}

export const NewAudit: React.FC<NewAuditProps> = ({ onAuditComplete, existingClaimId, initialCdtCode }) => {
  ...
  const [loadingClaim, setLoadingClaim] = useState(Boolean(existingClaimId));

  useEffect(() => {
    if (!existingClaimId && initialCdtCode) setCdtCode(initialCdtCode);
  }, [existingClaimId, initialCdtCode]);

  useEffect(() => {
    fetchPayers().then((res) => {
      setPayers(res);
      if (res.length > 0 && !existingClaimId) setSelectedPayerId(res[0].id);
    });
  }, [existingClaimId]);
  ...
  useEffect(() => {
    ...
    fetchClaimById(existingClaimId)
      .then((claim) => {
        const procedure = claim.procedures?.[0];
        setPatientId(claim.patient_id || '');
        setPatientName(claim.patient_name || '');
        setDob(claim.date_of_birth || '');
        setSelectedPayerId(claim.payer_id || '');
        setCdtCode(procedure?.cdt_code || 'D2740');
        setToothNumber(procedure?.tooth_number || '14');
        setDos(claim.date_of_service || '');
        setClaimAmount(String(claim.claim_amount || procedure?.amount || ''));
        setNarrative(claim.clinical_narrative || '');
        setUploadedDocs(claim.documents || []);
      })
      .catch(() => alert('Unable to load the claim for editing.'))
  ...
  const handleDocumentUpload = async (file: File, documentType: string) => {
    ...
    try {
      if (existingClaimId) {
        const uploaded = await uploadDocumentApi(existingClaimId, file, documentType);
  ...
  const handleRunAudit = async () => {
    ...
    try {
      ...
      if (existingClaimId) {
        await updateClaimApi(existingClaimId, {
      ...
      const auditResult = await runAuditApi(claimId!);
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-extrabold text-slate-900 tracking-tight">{existingClaimId ? 'Update Claim & Re-audit' : 'New Claim Pre-Submission Audit'}</h2>
            <p className="text-xs text-slate-500">Step {step} of 5: {existingClaimId ? 'Correct claim details and missing documents' : 'Fill parameters to execute live audit'}</p>
          </div>
      ...
      {loadingClaim ? (
        <div className="bg-white rounded-3xl border border-slate-200 shadow-xl p-12 text-center text-sm text-slate-500">Loading claim for editing...</div>
      ) : <>
      {/* Step Content Card */}
