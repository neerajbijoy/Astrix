...
async function triggerAudit(req, res, next) {
  try {
    const { id } = req.params;
    const auditResult = await runClaimAudit(id, req.demoUserId);
...
async function getLatestAudit(req, res, next) {
  try {
    const { id } = req.params;
    const claim = await db.getClaimById(id, req.demoUserId);
    if (!claim) {
    ...
    if (!claim.latest_audit) {
      ...
      const freshAudit = await runClaimAudit(id, req.demoUserId);
...
async function triggerReAudit(req, res, next) {
  try {
    const { id } = req.params;
    const previousClaim = await db.getClaimById(id, req.demoUserId);
    ...
    // Run new audit
    const newAuditResult = await runClaimAudit(id, req.demoUserId);
