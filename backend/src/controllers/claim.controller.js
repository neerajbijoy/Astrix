...
async function getClaims(req, res, next) {
  try {
    ...
    const claims = await db.getClaims({ status, payer_id, search, owner_id: req.demoUserId });
...
async function updateClaim(req, res, next) {
  try {
    ...
    if (tooth_number && claim.procedures && claim.procedures.length > 0) {
      const updatedProcedure = await db.updateProcedure(claim.procedures[0].id, { tooth_number });
      if (updatedProcedure) claim.procedures[0] = updatedProcedure;
    }
